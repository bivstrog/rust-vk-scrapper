...
mod test_utils;
use test_utils::{setup_test_db, insert_post, insert_post_info};
...
fn test_get_polling_with_data() {
    ...
    let post_id = rt.block_on(async {
        ...
        insert_post_info(&pool, post_id, 20, 10, 4, 200, now + chrono::Duration::minutes(30))
            .await
            .expect("Failed to insert post_info 2");
        
        insert_post_info(&pool, post_id, 30, 15, 6, 300, now + chrono::Duration::minutes(60))
            .await
