...
async fn test_poll_post_stats_calls_vk_and_saves_to_db() {
    ...
    assert_eq!(post_info_count, 1, "Should have created one POST_INFO entry");
...
async fn test_poll_post_stats_stops_when_ready_to_finish() {
    ...
    let result = poll_post_stats(&job_id, &scheduler, &pool, post_id).await;
    assert!(result.is_ok(), "poll_post_stats should succeed even when stopping");
    ...
    assert_eq!(post_info_count, 0, "Should NOT have created POST_INFO for expired post");
...
async fn test_init_all_tasks_starts_tasks_for_active_posts() {
    ...
    println!("  - Should have started task for post {} (active without recent polling)", active_post_id);
    println!("  - Should NOT have started task for post {} (active with recent polling)", active_with_recent_id);
    println!("  - Should NOT have started task for post {} (expired)", expired_post_id);
}
...
async fn test_init_all_tasks_with_no_posts() {
    ...
    let result = init_all_tasks(&pool, &scheduler).await;
    assert!(result.is_ok(), "init_all_tasks should succeed with no posts");
...
async fn test_poll_post_stats_multiple_calls_accumulate_data() {
    ...
    assert_eq!(post_info_count, 3, "Should have created 3 POST_INFO entries");
