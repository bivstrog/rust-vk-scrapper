...
use crate::db_commands::{is_ready_to_finish, get_vk_id_by_post_id, save_post_info, get_posts_needing_polling};
use crate::vk_api::call_vk;
use crate::utils::get_pooling_delta_seconds;

pub async fn init_all_tasks(pool: &PgPool, scheduler: &JobScheduler) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (db_post_id, vk_id) in posts {
        ...
        println!("Started polling task for post {} (db_id: {})", vk_id, db_post_id);
    }
    
    Ok(())
}

pub async fn poll_post_stats(job_id: &uuid::Uuid, locked_scheduler: &JobScheduler, pool: &PgPool, db_post_id: i32) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vk_id = get_vk_id_by_post_id(pool, db_post_id).await?
    ...
    println!("Successfully polled stats for post {}: likes={}, comments={}, reposts={}, views={}",
