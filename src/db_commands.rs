...
pub async fn get_vk_id_by_post_id(pool: &PgPool, post_id: i32) -> Result<Option<String>, sqlx::Error> {
    let result = sqlx::query(
        r#"
...
pub async fn has_recent_post_info(pool: &PgPool, post_id: i32, seconds: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query(
        r#"
        SELECT EXISTS(
...
pub async fn get_post_with_data(pool: &PgPool, scrapper_id: i32) -> Result<Option<PostWithData>, sqlx::Error> {
    ...
    let data: Vec<PostInfoData> = data_rows.iter().map(|row| PostInfoData {
